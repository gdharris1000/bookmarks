...
class Tag
    ...
    def self.where(bookmark_id:)
        result = DatabaseConnection.query("SELECT id, content FROM bookmarktags INNER JOIN tags ON tags.id = bookmarktags.tag_id WHERE bookmarktags.bookmark_id = '#{bookmark_id}';")
        result.map { | tag |
